...
const SignUpPage = ({userInfo, updateForm, toggleModal}) => {
  ...
  return (
    <form className={`${updateForm && "h-[90%] overflow-auto"} md:px-16 px-8 flex flex-col mx-auto my-9 items-center gap-4 w-fit rounded-lg bg-[#FFFFFF] shadow-xl dark:text-white dark:bg-[#141624]`}
    onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="w-full flex items-center justify-center flex-col my-4">
        {updateForm ? (<button disabled={updateProfileMutation.isPending} className="bg-[#4B6BFB] text-white w-full py-3 px-2 rounded-md flex items-center justify-center gap-2">
          {updateProfileMutation.isPending ? (<> <SmallSpinner/> <small className="text-[16px]">Updating Form.....</small></>) : 
          (<small className="text-[16px]">Update Profile Form</small>)}
        </button>): 
        (<button className="bg-[#4B6BFB] text-white w-full py-3 px-2 rounded-md flex items-center justify-center gap-2">
          ...
          (<small className="text-[16px]">Sign Up</small>)}
        </button>)}
        <p className="text-[14px]">Already have an account? <Link to="/signin">SignIn</Link></p>
      </div>
    </form>
  )
}
